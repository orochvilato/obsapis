...
def visuelvotecledetail21(num,fs=32,fst=34):
    ...
    scrutins_positions = use_cache('scrutins_positions',lambda:getScrutinsPositions(),expires=36000)
    scrutin = mdb.scrutins.find_one({'scrutin_num':num})
    fulldate = datetime.datetime.strptime(scrutin['scrutin_date'],'%d/%m/%Y').strftime('%e %B %Y').strip()
    ...
    scrutin['dossier'] = scrutin['dossier'].replace('\t','')
    y = drawwrappedtext(eval=True,img=d,txt=scrutin['dossier'],x=o_x+12,y=o_y+fontthemesize+24+4, font=fontdos, maxwidth=1024-o_x,lineheight=fontdossize+6,color=(255,255,255,255))
    d.rectangle(((o_x, o_y+fontthemesize+20), (1024-o_x,y+2)),fill=(33,53,88,255))
    y = 15+drawwrappedtext(img=d,txt=scrutin['dossier'],x=o_x+12,y=o_y+fontthemesize+24+4, font=fontdos, maxwidth=1024-o_x,lineheight=fontdossize+6,color=(255,255,255,255))
    ...
    fontposb = ImageFont.truetype("Montserrat-SemiBold.ttf", 16)
    fontpos  = ImageFont.truetype("Montserrat-Regular.ttf", 16)

    # votes
    for col, pos in enumerate(['Pour','Contre','Abstention']):
        d.text((o_x+col*(colwidth+colspace),y+10), pos, font=fontpos, fill=(33,53,88,255))
        pw,ph = fontpos.getsize(pos)
        if scrutin.get('inversion','non') == 'oui':
            _pos = 'contre' if pos == 'Pour' else 'pour' if pos == 'Contre' else pos.lower()
        else:
    ...
    # legende
    legendimg = Image.new('RGB',(440,800),color=(255,255,255))
    legenddraw = ImageDraw.Draw(legendimg)
    leg_fs = 35
    fontpos2  = ImageFont.truetype("Montserrat-Regular.ttf", leg_fs)
    for row,leg in enumerate(legend):
        legenddraw.ellipse((0,leg_fs/2+row*75,leg_fs,leg_fs/2+leg_fs+row*75),gpcolors[leg[0]])
        for i,l in enumerate(leg[1]):
            legenddraw.text((70,leg_fs/2+row*75+(i*leg_fs)-8-(leg_fs/2)*(len(leg[1])-1)),l, font=fontpos2, fill=(33,53,88,255))
    legimg = legendimg.resize((220,400),Image.ANTIALIAS).crop((0,0,220,300))
    vis.paste(legimg,(770,120))
