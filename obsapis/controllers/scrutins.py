...
def getScrutinsCles():
    ...
    for n in range(2):
        ...
        for e in elts:
            if e[2] != None:
                scles[int(e[0])] = dict(num=int(e[0]),nom=e[1],descfmt=markdown.markdown(e[2]),desc=e[2].replace('_','').replace('*',''),theme=e[3],lien=e[4],lien_texte=e[5],lien_source=e[6],dossier=e[7],niveau=n+1)
    return scles
...
def getScrutinsData():
    ...
    for s in mdb.scrutins.find({},{'scrutin_id':1,'scrutin_num':1,'_id':None,'scrutin_sort':1,'scrutin_urlAmendement':1,'scrutin_lientexte':1}):
        sdata[s['scrutin_num']] = {'sort':s['scrutin_sort'],'urlAmendement':s.get('scrutin_urlAmendement',None),'scrutin_lientexte':s.get('scrutin_lientexte',None)}
    return sdata
