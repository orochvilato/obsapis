...
def connectionsjson():
    ...
    depgp = dict((d['depute_shortid'],{'g':gps[d['groupe_abrev']],'n':d['depute_nom']}) for d in mdb.deputes.find({},{'depute_nom':1,'depute_shortid':1,'groupe_abrev':1,'_id':None}))
    ...
    for i,d in enumerate(allitems):
        r['nodes'].append({'id':d,'name':depgp[d]['n'],'group':depgp[d]['g']})
        deputes[d] = i
    for l,n in Counter(frozenset(x) for x in liens).items():
        if len(list(l))==2 :
            s,t = list(l)
            if depgp[s]['g'] in gsel or depgp[t]['g'] in gsel:
