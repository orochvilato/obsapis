...
class AssembleeSpider(scrapy.Spider):
    ...
    def parse_depute(self, response):
        ...
        if finmand:
            ...
            place = response.xpath('//div[@id="hemicycle-container"]/@data-place').extract()
            if place:
                place=place[0]
            else:
                place="0"
        ...
        permanences=[]
        for perm in response.xpath('//dd/dl[dt[contains(text(),"circonscrip")]]'):
            permanence = {
                'rue':[ adr.split(': ')[-1].replace('\n','') for adr in perm.xpath('*[@class="street-address"]/text()').extract()],
                'cp':perm.xpath('dd/*[@class="postal-code"]/text()').extract()[0],
                'ville':perm.xpath('dd/*[@class="locality"]/text()').extract()[0]
            }
            permanences.append(permanence)
        print nom,permanences
        deputes[uid] = dict(
            depute_actif = actif,
            depute_nom = nom,
            depute_permanences=permanences,
        ...
        if not debmand:
            del deputes[uid]['depute_mandat_debut']
